...
begin
  while (v_id <= 3500000) loop
      ...
      INSERT INTO InfoProdus(id_produs, id_user, tip, achizitie, pret, suprf_utila) values (v_id, v_id_user, v_tip, v_achizitie, v_pret, v_suprf_utila);
      v_id := v_id +1;
  end loop;
commit;
end; 
...
begin
  ...
  while (v_id_extra <= 3500000) loop
      ...
      INSERT INTO ExtraInfoProdus(id_extra, id_produs, nr_camere, nr_bai, descriere, confort, vechime, niveluri_imobil, etaj, tip_teren) values (v_id_extra, v_id_produs, v_nr_camere, v_nr_bai, v_descriere, v_confort, v_vechime, v_niveluri_imobil, v_etaj, v_tip_teren);
